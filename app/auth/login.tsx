...
export default function Login() {
  ...
  const handleLogin = async () => {
    if (!email || !password) {
      Alert.alert(i18n.t('auth.errorTitle'), i18n.t('auth.errorEmailPassword'))
      return
    }
    ...
    try {
      ...
      if (data.token) {
        ...
        Alert.alert(i18n.t('auth.errorTitle'), data.message || i18n.t('auth.errorLogin'))
      }
    } catch {
  ...
  return (
    <View style={styles.container}>
      ...
      <TouchableOpacity
        ...
        disabled={isLoading}
      >
        <Text style={styles.buttonText}>{isLoading ? i18n.t('auth.loggingIn') : i18n.t('auth.login')}</Text>
      </TouchableOpacity>
